...
describe('CorrectionsPage', () => {
  it('summarizes correction impact without implying raw Garmin mutation', () => {
    ...
    expect(within(screen.getByLabelText('订正目标分布')).getByText('球洞 2')).toBeInTheDocument()
    expect(within(screen.getByLabelText('订正目标分布')).getByText('球局 1')).toBeInTheDocument()
    expect(within(screen.getByLabelText('订正目标分布')).getByText('击球 1')).toBeInTheDocument()
