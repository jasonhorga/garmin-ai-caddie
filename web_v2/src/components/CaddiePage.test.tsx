...
describe('CaddiePage', () => {
  ...
  it('renders decision evidence and requests a source-bound plan', async () => {
    ...
    expect(screen.getAllByText('样本 24').length).toBeGreaterThan(0)
    expect(screen.getAllByText('覆盖 4/4').length).toBeGreaterThan(0)
    expect(screen.getAllByText('高 方案置信度').length).toBeGreaterThan(0)
    expect(screen.getByText('历史 +0.8 风险')).toBeInTheDocument()
    expect(screen.getAllByText('已选').length).toBeGreaterThan(0)
    // 144m→157码, 16m→17码 (Math.round(16*1.09361)=Math.round(17.498)=17)
    expect(screen.getByText('157码 - 风险 1 - 1.24 预期 - 17码 余量')).toBeInTheDocument()
    expect(screen.getAllByText('water_front').length).toBeGreaterThan(0)
    expect(screen.getAllByText('wind').length).toBeGreaterThan(0)
    expect(screen.getAllByText('中 置信度').length).toBeGreaterThan(0)
    expect(screen.getByText('execution')).toHaveClass('audit-execution')
    expect(screen.getByText('计划 stock → 实际 stock')).toBeInTheDocument()
    ...
    expect(screen.getByText('选杆一致 是')).toBeInTheDocument()
    // -1m → -1码 (Math.round(-1*1.09361) = -1)
    expect(screen.getByText('距离 -1码')).toBeInTheDocument()
    expect(screen.getByText('风险 否')).toBeInTheDocument()
    ...
    expect(within(scoreImpact).getByRole('heading', { name: '得分影响' })).toBeInTheDocument()
    ...
    expect(within(scoreImpact).getByText('+0.24 杆')).toBeInTheDocument()
    expect(within(scoreImpact).getByText('history +0.12 杆')).toBeInTheDocument()
    expect(within(scoreImpact).getByText('clubConfidence +0.02 杆')).toBeInTheDocument()
    ...
    expect(within(explanationPanel).getByRole('heading', { name: '决策说明' })).toBeInTheDocument()
    ...
    expect(within(explanationPanel).getByText('needs_review 绑定')).toBeInTheDocument()
    ...
    const factsHeading = within(explanationPanel).getByRole('heading', { name: '依据' })
    ...
    expect(screen.getByText('风 19 km/h')).toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: '逐杆序列' })).toBeInTheDocument()
    expect(screen.getByText('1D-3W-58')).toBeInTheDocument()
    expect(screen.getAllByText(/3 杆/).length).toBeGreaterThan(0)
    expect(screen.getByText('覆盖 153/153')).toBeInTheDocument()
    expect(screen.getByText('高 序列置信度')).toBeInTheDocument()
    expect(screen.getByText('advance 1D')).toBeInTheDocument()
    // 245m→268码 (Math.round(267.93)=268), 275m→301码 (Math.round(300.74)=301)
    expect(screen.getByText('268码 带球 - 301码 剩余')).toBeInTheDocument()
    expect(screen.getByText('80 样本')).toBeInTheDocument()
  ...
  it('captures sequence audit shots, penalty, and score context', async () => {
    ...
    await userEvent.clear(screen.getByLabelText('实际球杆'))
    await userEvent.type(screen.getByLabelText('实际球杆'), '1D')
    await userEvent.clear(screen.getByLabelText('实际带球 (米)'))
    await userEvent.type(screen.getByLabelText('实际带球 (米)'), '242')
    await userEvent.selectOptions(screen.getByLabelText('落位'), 'fairway')
    await userEvent.clear(screen.getByLabelText('第2杆球杆'))
    await userEvent.type(screen.getByLabelText('第2杆球杆'), '3W')
    await userEvent.type(screen.getByLabelText('第2杆带球 (米)'), '211')
    await userEvent.selectOptions(screen.getByLabelText('第2杆落位'), 'fairway')
    await userEvent.clear(screen.getByLabelText('第3杆球杆'))
    await userEvent.type(screen.getByLabelText('第3杆球杆'), '58')
    await userEvent.type(screen.getByLabelText('第3杆带球 (米)'), '74')
    await userEvent.selectOptions(screen.getByLabelText('第3杆落位'), 'green')
    await userEvent.type(screen.getByLabelText('实际相对标准杆'), '1')
    await userEvent.click(screen.getByLabelText('发生罚杆'))
    await userEvent.click(screen.getByRole('button', { name: '复盘结果' }))
