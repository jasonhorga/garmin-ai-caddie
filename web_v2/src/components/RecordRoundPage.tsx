...
import { courseSegmentName, displayCourseName } from '../courseName'
...
export function RecordRoundPage({ playerId, playerName, courseOptions, onIngest, getPosition, onExit }: RecordRoundPageProps) {
  ...
  const selectedCourse = courses.find((course) => course.globalId === courseGlobalId) ?? null
  const selectedCourseName = selectedCourse ? displayCourseName(selectedCourse) : '未指定球场'
  ...
  async function submitRound() {
    ...
    try {
      const meta: Record<string, unknown> = {
        // A manual GPS round may be recorded without a selected Garmin course, but its
        // label is generated by the product rather than typed by the player. This keeps
        // free-form names out of the shared course catalogue/identity path.
    ...
    return (
      <section className="record-page" aria-label="记分完成">
        <section className="panel record-done">
          <h1>已提交 ✅</h1>
          <p>
            {selectedCourseName} · {result.holesCompleted ?? scoredHoles} 洞
        ...
        <section className="panel record-setup">
          {courses.length > 0 ? (
            <label className="record-field">
              ...
              >
                <option value="">不指定球场</option>
                {courses.map((c) => (
                  <option key={c.globalId} value={c.globalId}>
                    {displayCourseName(c)}{courseSegmentName(c) ? ` · ${courseSegmentName(c)} 场` : ''}
                  </option>
                ))}
              </select>
            </label>
          ) : (
            <p className="record-field-hint">没有可用的 Garmin 球场，本次记录将保存为未指定球场。</p>
          )}
  ...
  return (
    <section className="record-page" aria-label="记分中">
      <header className="overview-hero record-hole-head">
        <div>
          <p className="eyebrow">{selectedCourseName}</p>
