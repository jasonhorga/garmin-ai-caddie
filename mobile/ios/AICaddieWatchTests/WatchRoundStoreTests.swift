...
final class WatchRoundStoreTests: XCTestCase {
    ...
    func testRecordIgnoresReplayOfSameEventId() throws {
        let (store, _) = tempStore()
        try store.upsertHoleState(holeState(hole: 1, score: 0))
        _ = try store.record(event("e1", hole: 1, kind: .score, value: "4"))
        let replay = try store.record(event("e1", hole: 1, kind: .score, value: "7"))

        XCTAssertEqual(replay.pendingEvents.map(\.eventId), ["e1"])
        XCTAssertEqual(store.holeState(hole: 1)?.score, 4)
    }
