...
final class WatchCourseDownloadTests: XCTestCase {
    ...
    func testNearbyWithoutConfigReportsAuthenticationBoundary() async {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("watch-discovery-(UUID().uuidString)", isDirectory: true)
        let library = WatchCourseLibrary(store: WatchCourseStore(directoryURL: directory))

        await library.refreshNearby(
            latitude: 40.0,
            longitude: 116.0,
            config: nil
        )

        XCTAssertEqual(library.discoveryState, .needsAuthentication)
        XCTAssertTrue(library.nearbyCourses.isEmpty)
        XCTAssertEqual(library.errorMessage, "请先在 iPhone 登录并同步一次")
    }

    func testImmediateStartCreatesPendingRoundWithoutNetworkAndPersistsTemplate() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("watch-bootstrap-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertEqual(library.errorMessage, nil)
        XCTAssertNotNil(
            WatchCourseStore(directoryURL: directory).course(globalId: 7001),
            "the pending template must survive a Watch process interruption"
        )
    }
    ...
    func testBuildPrefersSharedTopoBitmapOverLegacyPrepJPEG() throws {
        ...
        let package = try client.decodeCoursePackage(Data(
            #"{"roundId":"watch-topo-v10","course":{"globalId":31669,"name":"北京丽宫","teeBox":"Blue"},"holes":[{"number":1,"par":4,"yards":404,"geometryCoverage":"ready","sourceGlobalId":31669,"sourceLocalHole":1}]}"#.utf8
        ))
        let prep = try client.decodeCoursePrep(Data(
