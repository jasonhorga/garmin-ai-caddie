...
public struct WatchHoleMapView: View {
    ...
    /// Return the actual perimeter distance for the score ring. The rounded display is not a square,
    /// so a fixed 25% offset does not land on 3 o'clock on every 41/45/49 mm face. The perimeter
    /// helper below starts at 12 o'clock and travels clockwise; this converts the legacy normalized
    /// 0.25...1.0 contract into an exact right-midpoint -> top-midpoint sweep.
    static func scoringRingDistance(
        index: Int,
        count: Int,
        halfW: CGFloat,
        halfH: CGFloat,
        ...
        let flatW = max(0, halfW - r)
        let flatH = max(0, halfH - r)
        let arc = CGFloat.pi * r / 2
        let perimeter = 4 * flatW + 4 * flatH + 2 * CGFloat.pi * r
        // From the top midpoint to the right midpoint along the clockwise path.
        let rightMidpoint = flatW + arc + flatH
        let normalized = scoringRingCenterFraction(index: index, count: count)
        let progress = min(max((normalized - 0.25) / 0.75, 0), 1)
        return rightMidpoint + (perimeter - rightMidpoint) * progress
    }
    ...
    /// Garmin-style 999 sentinel so it cannot be mistaken for a measured range.
    ...
    /// the bottom menu/shot controls or the hardware mask.
    static func rootCaddieChipFrame(in size: CGSize) -> CGRect {
        let safeRect = WatchDisplayGeometry.contentRect(in: size)
        let width: CGFloat = min(56, max(50, safeRect.width * 0.30))
        let height: CGFloat = 36
        // The root keeps the menu and manual-shot controls in the bottom instrument rail. Leave a
            ...
            if fullMap {
                ...
                if showTextOverlay, showCaddieRecommendation {
                    ...
                    .position(x: Self.rootCaddieChipFrame(in: size).midX,
                              y: Self.rootCaddieChipFrame(in: size).midY)
                }
            }
    ...
    private func centerRangeLabel(_ centerGreen: Int) -> String {
        if rangeUnavailable { return "999 码 · 等待定位" }
        if WatchGeoMath.isBeyondUsefulGreenRange(centerGreen) { return "离本洞较远" }
        return "中 \(WatchGeoMath.greenRangeText(centerGreen)) 码 · 到果岭"
    }
    ...
    private func drawRing(_ context: inout GraphicsContext, size: CGSize) {
        ...
        let ringStart = fw + CGFloat.pi * r / 2 + fh
        let ringEnd = perim
        for (index, pip) in ringPips.enumerated() {
            let s = Self.scoringRingDistance(
                index: index,
                count: count,
                halfW: halfW,
                halfH: halfH,
            ...
            for k in 0...n {
                let ss = min(
                    max(s - segHalf + (2 * segHalf) * CGFloat(k) / CGFloat(n), ringStart),
                    ringEnd
                )
