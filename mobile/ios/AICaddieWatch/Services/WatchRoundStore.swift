...
public final class WatchRoundStore {
    ...
    public func record(_ event: WatchInputEvent, updateActiveHole: Bool = true) throws -> PersistedRound {
        ...
        // Event IDs are the idempotency key shared with the phone/backend. Replayed delivery (or a
        // double tap while the Watch UI is transitioning) must not apply a second field update or
        // append a second ownership-bearing fact.
        if round.pendingEvents.contains(where: { $0.eventId == event.eventId }) {
            return round
        }
