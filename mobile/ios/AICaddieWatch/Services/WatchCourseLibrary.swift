...
/// Explicit nearby-discovery lifecycle. A missing config is a provisioning/auth boundary, not a
/// successful provider lookup; a missing GPS fix is a location boundary.
public enum WatchCourseDiscoveryState: Equatable {
    case waitingForGPS
...
public final class WatchCourseLibrary: ObservableObject {
    ...
    public func refreshNearby(
        ...
        guard let config else {
            ...
            discoveryState = cachedNearby.isEmpty ? .needsAuthentication : .ready
        ...
        do {
            ...
            var seen = Set<Int>()
            let resolved = matches.compactMap { resolvedNearbyOption($0) }.filter {
                seen.insert($0.globalId).inserted
            }
            nearbyCourses = resolved
            discoveryState = .ready
            // Retain provider-discovered metadata for setup and later offline starts. This does not
            // make the rows a history list; WatchStartView only presents these entries in the nearby
            // section while a location is available.
            courses = Self.uniqueOptions(from: resolved + courses)
            ...
            guard !Task.isCancelled, nearbyRequestToken == requestToken else { return }
            nearbyCourses = cachedNearby
            discoveryState = cachedNearby.isEmpty ? .error : .ready
    ...
    public func startCourse(
        ...
        guard preparingCourseId == nil else { return nil }
    ...
    /// Create the local round immediately. A Watch round is a durable player action, not a course
    /// download result: when the wearer is away from the venue or the phone is asleep, the round
