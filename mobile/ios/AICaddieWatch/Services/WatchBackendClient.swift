...
public final class WatchBackendClient {
    ...
    public static let topoStyleVersion = "topo-v11"
    ...
    public func cancelCourseInstallJob(globalId: Int, jobId: String) async throws -> WatchCourseInstallStatus {
        try await postCourseInstallJobAction(globalId: globalId, jobId: jobId, action: "cancel")
    }

    public func retryCourseInstallJob(globalId: Int, jobId: String) async throws -> WatchCourseInstallStatus {
    ...
    private func postCourseInstallJobAction(
        ...
        applyAuth(&request)
        let data = try await sendForData(request, retryingTransientFailures: false)
        return try decodeCourseInstallStatus(data)
    }
