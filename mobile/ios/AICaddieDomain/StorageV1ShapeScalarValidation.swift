...
internal enum StorageV1ShapeScalarValidation {
    ...
    internal static func validateRecursiveNumber(
        ...
        } catch CanonicalJSONError.unsafeInteger {
            canonical = try validatedCanonicalExponentSpelling(
                value: value,
                rawBytes: rawBytes
            )
        } catch {
    ...
    private static func validatedCanonicalExponentSpelling(
        value: Double,
        rawBytes: Data
    ) throws -> Data {
        let spelling = String(value)
        guard let marker = spelling.firstIndex(of: "e"),
              let exponent = Int(spelling[spelling.index(after: marker)...]),
              exponent >= 21 else {
            throw StorageV1ShapeCodec.ValidationError.invalidNumber
        }
        let canonical = Data(spelling.utf8)
        guard canonical == rawBytes else {
            throw StorageV1ShapeCodec.ValidationError.invalidNumber
        }
        return canonical
