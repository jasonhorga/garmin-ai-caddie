            ...
            Group {
                if requiresSignIn {
                    ...
                    RoundHomeView(
                        ...
                        garminConnectionState: model.garminConnectionState,
...
public final class LiveRoundAppModel: ObservableObject {
    ...
    @Published public private(set) var garminConnectionState: GarminConnectionState = .disconnected
    /// Compatibility/read-only copy for existing call sites and tests. The enum above is the only
    /// mutable source; localized text can no longer drift from account state.
    ...
    public func refreshGarminSyncPresentation() async {
        ...
        if lastRun.errorCode == "session_stored"
            ...
            case "ready":
                garminConnectionState = lastRun.newRoundCount == 0
                    ? .connectedNoChanges
                    : .connectedUpdated
            case "no_data":
            ...
            case "error":
                garminConnectionState = hasVerifiedGarminSession() ? .syncFailed : .verificationFailed
            case "not_available":
                garminConnectionState = hasVerifiedGarminSession() ? .syncFailed : .verificationFailed
            default:
                break
    ...
    public func autoSyncGarminIfNeeded() async {
        ...
        for _ in 0..<24 {
            ...
            await refreshGarminSyncPresentation()
            if garminConnectionState == .reauthRequired {
                return
            }
        }
    }
    ...
    /// Compatibility wrapper for older callers. A Garmin account is considered connected only when
    /// the typed operation returns `.completed`; an occupied lock, expired session, or transport
    /// failure must remain distinguishable to the account UI.
    @discardableResult
    ...
    @discardableResult
    public func syncGarminDataOutcome() async -> GarminSyncOutcome {
        if let garminSyncTask {
            return await garminSyncTask.value
        }
        garminSyncOperationGeneration += 1
        let operationGeneration = garminSyncOperationGeneration
        let task: Task<GarminSyncOutcome, Never> = Task { [weak self] in
    ...
    private func performGarminSync(operationGeneration: Int) async -> GarminSyncOutcome {
        ...
        do {
            ...
            guard markGarminSessionVerified(after: completedAt) else {
                garminConnectionState = .persistenceFailed
                garminSyncPresentationLockedUntil = Date().addingTimeInterval(3)
                garminSyncPresentationWatermark = Date()
                return .failed
            }
            lastGarminSyncAt = completedAt
            garminConnectionState = refreshedStatus?.lastRun?.newRoundCount == 0 || result.state == "no_data"
                ? .connectedNoChanges
                : .connectedUpdated
    ...
    private func storedGarminConnectionState() -> GarminConnectionState {
        guard let garminSessionStore,
              let material = try? garminSessionStore.loadSession() else {
            return .disconnected
        }
        return material.verifiedAt == nil ? .awaitingVerification : .connected
    }
    ...
    /// A confirmed Garmin auth failure invalidates the local badge as well as the server status. Keep
    /// no expired cookie around: reopening the account screen must not turn a past success into a
    /// false "已连接" state.
    private func clearStoredGarminSessionAfterAuthFailure() {
        guard let garminSessionStore else { return }
        do {
            try garminSessionStore.deleteSession()
        } catch {
            AICaddieLog.storage.error("Expired Garmin session could not be cleared: \(String(describing: error), privacy: .public)")
        }
    }
