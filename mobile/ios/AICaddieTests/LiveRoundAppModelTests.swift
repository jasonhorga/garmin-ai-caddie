...
final class LiveRoundAppModelTests: XCTestCase {
    ...
    func testConcurrentGarminSyncCoalescesAndStaleServerFailureCannotRepaintSuccess() async throws {
        SessionStore.shared.signOut()
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let configuration = URLSessionConfiguration.ephemeral
        ...
        CapturingURLProtocol.requestHandler = { request in
            ...
            case ("POST", "/api/v2/sync/garmin"):
                ...
                // Give the second caller time to enter while the shared operation is in flight.
            ...
            case ("GET", "/api/v2/mobile/courses/options"):
                payload = Data(
                    #"{"schema":"ai-caddie-mobile-course-options-v1","dataMode":"production","total":0,"courses":[],"generatedAt":"2026-08-22T08:00:00Z"}"#.utf8
                )
            case ("GET", "/api/v2/history/clubs/bag"):
                payload = Data(#"{"found":false,"clubs":[]}"#.utf8)
            case ("GET", "/api/v2/sync/status"):
                // Deliberately model the lagging row the production UI used to consume after a
                // successful POST. Its old timestamp must remain below the local completion watermark.
                payload = Data(
                    #"{"schema":"ai-caddie-sync-status-v2","lastRun":{"state":"error","detail":"old failure","updatedAt":"2026-08-21T00:00:00Z"}}"#.utf8
                )
            default:
                XCTFail("unexpected request \(request.httpMethod ?? "GET") \(url.path)")
                payload = Data(#"{"detail":"unexpected"}"#.utf8)
        ...
        async let first = model.syncGarminData()
        async let second = model.syncGarminData()
        let (firstResult, secondResult) = await (first, second)

        XCTAssertTrue(firstResult)
        XCTAssertTrue(secondResult)
        XCTAssertEqual(requestLock.withLock { garminPOSTCount }, 1)
        XCTAssertEqual(model.garminSyncStatus, "Garmin 数据已更新")

        await model.refreshGarminSyncPresentation()

        XCTAssertEqual(
            model.garminSyncStatus,
            "Garmin 数据已更新",
            "a lagging server row must not repaint a newer local success"
        )
    }
    ...
    func testFastStartRefreshRetriesAfterTransientFullPackageFailure() async throws {
        ...
        let fast = package(
            ...
            recentRounds: [],
            holes: [first],
            startMode: "first_hole_fast",
            fullCoursePending: true
        ).replacingCoursePrep(nil)
        let complete = package(
            source,
            roundId: roundId,
        ...
        CapturingURLProtocol.requestHandler = { request in
            ...
            let isFast = URLComponents(url: url, resolvingAgainstBaseURL: false)?.queryItems?
                .first(where: { $0.name == "fast_start" })?.value == "true"
            let response: (HTTPURLResponse, Data)
            requestLock.lock()
            ...
            if isFast {
                ...
                if fullRequestCount == 1 {
                    response = (
                        ...
                        Data(#"{"detail":"still preparing"}"#.utf8)
                    )
                } else {
                    response = (
                        HTTPURLResponse(
        ...
        let client = SyncClient(
            baseURL: URL(string: "https://fast-start-retry.example.test")!,
            session: session,
            retrySleep: { _ in XCTFail("non-transient handoff failure must not retry in SyncClient") }
        )
        let model = LiveRoundAppModel(
            offlineStore: OfflineStore(directoryURL: directory),
            apiBaseURL: client.baseURL,
        ...
        XCTAssertEqual(requestLock.withLock { fastStartFlags }, [true, false, false])
        XCTAssertEqual(model.package?.holes.count, source.holes.count)
    ...
    func testFastStartScoreAdvanceWaitsForFullPackageThenUsesNextHole() throws {
        ...
        XCTAssertEqual(
            LiveHoleAdvanceResolution.resolve(after: first.number, package: fast),
            .waitForFullCourse(nextHole: first.number + 1)
        )
        ...
        XCTAssertEqual(
            LiveHoleAdvanceResolution.resolve(after: first.number, package: complete),
            .advance(to: source.holes[1].number)
        )
    }
