...
printf 'name\tbytes\tmtime\tnewest_file\tgit\tvenv\tnode_modules\tbuild_artifacts\tevidence_artifacts\tdata_artifacts\tlink_count\n' > "$OUT/runs.tsv"
...
for dir in "$RUNS"/*; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    "$build" "$evidence" "$data" "$links" >> "$OUT/runs.tsv"
done
