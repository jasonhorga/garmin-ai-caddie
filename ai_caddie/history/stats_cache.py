...
input ``build_history_stats`` reads is covered ->
  ...
  - the four single-file aux stores (annotations / weather / reports / decision audits),
    resolved in the requesting player's evidence partition, exactly like the loaders.
...
def _aux_files(
    ...
    *,
    ...
    """The single-file aux stores build_history_stats reads (via list_* helpers) for ``player_id``.

    ``build_history_stats`` reads these through ``list_annotations``/``list_weather_snapshots``/
    ``list_report_stats_records``/``list_decision_audits``, all of which resolve the file under
    ``evidence_root(player_id, root=...)``. Fingerprinting the owner-flat paths for every player was
    wrong in both directions: a member's own note/weather/report/decision-audit write left their
    cached stats stale, and an unrelated owner write recomputed every member. Owner behaviour is
    unchanged — ``evidence_root(OWNER_ID, root=r)`` is exactly the ``Path(r or ".")`` these helpers
    already received."""
    return [
        annotation_file(evidence_root(player_id, root=annotations_root)),
        weather_snapshot_file(evidence_root(player_id, root=weather_root)),
        report_store_file(evidence_root(player_id, root=reports_root)),
        decision_audit_file(evidence_root(player_id, root=decision_audit_root)),
    ]
...
def _fingerprint(data, roots: dict[str, Any], player_id: str = OWNER_ID) -> tuple:
    ...
    return (
        ...
        tuple(_file_sig(f) for f in _aux_files(player_id=player_id, **roots)),
    )
