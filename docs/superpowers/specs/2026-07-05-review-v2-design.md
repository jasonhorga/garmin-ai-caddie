...
- **研究已出结论:可行**(Fable 独立查证,完整报告 `2026-07-05-auto-swing-detection.md` / funnel `/swing.html`)。要点:① 有量产先例(Golfshot 在 S3/watchOS7 就用 ML 全自动记杆、含推杆);② Apple 给了 800Hz 高频传感器接口 + WWDC 点名 golf + 现成的击球冲击检测算法;③ 真正难点不是「检测挥杆」(全挥杆好认),而是短打/推杆漏检 + 练习挥误检 —— 这恰好被我们已有的 **GPS 位移 + 球洞几何 + 自测球杆距离 + 逐洞记分** 四样数据补上(仲裁 + 每洞对账);④ 建议「**自动检测 + 每洞确认**」混合,别追纯自动;第一步纯软件、所有机型:现有 Watch 挂 golf workout + 100Hz 规则式检测 + 震动一键确认,顺便攒训练标注。
