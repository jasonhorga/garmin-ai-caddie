...
**版本 v1.0** · 更新 2026-07-05 · 主线 `integration/v2` · 总体完成度 **~70%**

> 这是**唯一的、活的**产品 Spec,**在主线 `integration/v2` 上就地维护**(不再散落分支)。
> **每次改动都要:① 记「改动日志」(§5) · ② 更新「完成度总览」(§0) · ③ 维护「TODO」(§6) · ④ 有实质更新就 bump 版本号 + 日期。**
...
| 模块 | 完成度 | 状态 |
|---|---|---|
| Web 重做(5 屏 + 外壳 + 真 topo 图) | **100%** | ✅ 已上线 `/me` |
| iOS 重做(打球暗 + 其余浅) | **~90%** | ✅ 主屏全落地(TestFlight 门控);尾:球包 gapping 阶梯 |
| 后端共享层(topo 渲染+缓存+预热 / 球杆解析) | **~60%** | 🔧 渲染管线+缓存已上;「填满取景」+ 球杆解析进行中 |
| shot-map 复盘(粗线 + 标球杆) | **0%** | ⬜ 待做(Web + iOS,吃后端共享层) |
| 开局统一预热(iOS/手表预加载) | **~30%** | 🔧 Web prewarm 已上;iOS/手表待接 |
| Apple Watch 重做 | 设计定稿 | 🔒 整合门控(等负责人) |

> 版本历史 = §5 改动日志(每次合并即一次记录)。完成度随 §5、§6 同步更新。
...
| (本 PR) | spec | **总 Spec 进主线 `integration/v2`** + 版本管理(v1.0)+ 完成度总览(§0);之前只在分支本地未 push→已同步 GitHub | 进行中 |
