...
| 5B storage-v1 literal schema | `IN_PROGRESS` — 5B1 and 5B2a-R `VERIFIED`; 5B2a-S `PENDING` | Final Task-5 literal record roster and required storage-v1 root encode/decode strictly without a single-receipt mutation surface; [design](../specs/2026-07-25-plan1-task5b-storage-v1-schema-design.md); [5B1 verification](../reviews/2026-07-25-plan1-task5b1-storage-v1-literals-verification.md); [5B2a-R candidate `2a8beb1` verification](../reviews/2026-07-25-plan1-task5b2a-r-raw-json-gate-verification.md) | `DomainRoundEvent.swift`, `DomainLedgerStateV1.swift`, `LegacyV1Transport.swift`, authority exception/pins, codec tests | 5A | file ownership, mutation, network, lifecycle |
...
## Frozen 5B serial subpacket ledger

These packets are deliberately serial. A typed `Codable` value is not a
validated storage capability, and no packet before 5B2b-T exposes a supported
storage-v1 decoder.
...
| 5B2a-R raw JSON gate | `VERIFIED` | Byte/depth/duplicate-key/key-length and absolute raw-string checks produce a non-public validated-raw capability with scalar-length evidence; [candidate `2a8beb1` verification](../reviews/2026-07-25-plan1-task5b2a-r-raw-json-gate-verification.md) | typed path/shape, ordinary-string policy, graph validation, algorithms |
...
| 5B2b-T algorithm-free transport graph | `PENDING` | Binding/prepared/Watch literal relationships and the sole supported composed V1 decoder | identity/hash recomputation, wire/binding/request algorithms, mutation/network |
...
- **Current drill-down:** POP from verified 5B2a-R; activate only the 5B2a-S
  generated V1 shape/codec.
- **Next:** write the bounded 5B2a-S card before its TDD cycle; do not absorb
  graph, algorithm, supported-decoder, mutation, or network work.
- **Completion boundary:** Task 5B, Task 5 and Plan 1 remain incomplete and
  unfrozen.
- **Owner decision:** none.
