...
- **PHONE-UX6 implementation checkpoint (2026-09-20):** The canonical prep/live/Watch
  ...
  corrected in the working tree and requires another source/Native gate before any
  TestFlight claim.
...
| `PHONE-UX6` | `in-progress` | Implement the `IMG_8135`-`IMG_8139` slice: canonical structured prep/live caddie plans with all landing legs, selectable plan-map state, removal of the offline-ready banner, coherent local-first round loading, and iPhone/Watch identity/interaction parity. Keep download-speed diagnosis measured and separate from UI claims. | Source commit `5681c6bd` and the follow-up focused suite (`323 tests, 2 skips`) passed. Native `35503821105` found the Codable/Watch issues; commit `79e5ddca` fixed them. Native `35505364421` then found a Swift named-argument ordering error in `CourseReviewView`; the one-line ordering fix is pending the next source/exact-SHA Native run. Screenshots live under `/home/ubuntu/IMG_*`. |
