...
> live task queue.
>
> **Resume contract:** the current slice, execution plan, evidence, blockers,
> and next action are authoritative only in this file. A session-memory plan is
...
(179 yd each), with the second full swing drawn through the green. The source
correction now hydrates stale live seeds from installed package facts and
preserves only the verified CoursePrep prefix when the measured live route is
shorter. The real Black Knight A1 package returns `Driver -> 3H`, omits the
stale through-green leg, and preserves shorter tee clubs when measured
water/OB/dispersion constraints justify them. Source, exact-SHA Native,
TestFlight upload, and Apple status gates are green. The screenshot is
`/home/ubuntu/IMG_8160.png`.

`PHONE-UX7` was previously `evidence-open` after the
...
- **IMG-8160 decision verification resource (2026-09-21 11:19 UTC):** This
  session owns one disposable remote source scratch at
  `/home/jason/codex-runs/garmin-ai-caddie-img8160-20260921-a`, expiring
  2026-09-22 11:19 UTC, and one auto-removed test container named
  `codex-img8160-decision-tests-20260921`. It mounts only the changed decision
  module and focused tests into the existing API image; it creates no volume,
  port, tunnel, service, dependency environment, or database. The candidate
  API container and its tunnel are pre-existing and must remain untouched.

- **IMG-8160 decision fix checkpoint (2026-09-21 11:42 UTC):** The working
  decision layer now rejects materially stale/repeated CoursePrep chains,
  tries distinct continuation clubs before a documented last-resort repeat,
  gives normal Par 4/5 tee advancement a bounded risk margin, and allows a
  tee-only 20m median-carry overshoot so a valid `1W -> 3H` line is not dropped
  as empty. The exact Black Knight A1-shaped regression selects `stock`,
  returns `1W -> 3H`, rejects the stale `58` leg and emits `tee_advancement`.
  Homeserver evidence: decision layer `87/87`, caddie golden replay `12/12`,
...
- **IMG-8160 candidate verification (2026-09-21 15:16 UTC):** Immutable
  candidate source `837c0d9a26bcfc6a4990e0778664135408d792e4` is healthy on
  loopback `127.0.0.1:39076` in container
  `aicaddie-release-837c0d9a-candidate-20260921`; Source CI `35615806302`
  passed all jobs. Authenticated sparse old-seed replay no longer returns
  `3H -> 3H` (it degrades to `Driver -> 5I` when route facts are absent).
  The real package seed, containing the current bag, 377-yard hole, route
  evidence and CoursePrep plan, returns `Driver -> 3H`, with
  `completion=replan_required`; the stale `58` leg is omitted because the
  measured live route ends before it. A fresh HTTP/2 Quick Tunnel forwards
  only this candidate at
  `https://battery-chan-metro-liverpool.trycloudflare.com`, owned by tmux
  `codex-img8160-prefix-tunnel-20260921`, metrics port `20250`, logs at
  `/home/jason/garmin-ai-caddie-data/operations/img8160-prefix-tunnel-20260921`,
  expiry `2026-09-22 15:10 UTC`. Exact-SHA Native Mobile run `35617605192`
  was dispatched from `integration/v2` with full capture, live preflight and
  this API origin; GitHub reports head SHA
  `837c0d9a26bcfc6a4990e0778664135408d792e4`.

- **IMG-8160 exact-SHA Native gate (2026-09-21 16:21 UTC):** Run
  `35617605192` completed successfully at exact source
  `837c0d9a26bcfc6a4990e0778664135408d792e4`. It passed iOS compilation and
  tests, authenticated live preflight, the real iPhone XCUITest journey, all
  Watch tests and runtime captures, design checks, evidence generation,
  artifact uploads and secret scans. The retained Black Knight evidence
  records `Driver,3H`; the real package decision evidence records
  `planSource=course_prep_prefix`, `truncated=true`, and
  `completion=replan_required`, so no third leg is drawn through the green.
  Native artifacts are `real-screenshots` `10650142949`, `real-video`
  `10649718144`, `watch-real-screenshots` `10651205404`, and
  `native-build-evidence` `10651660086`. The subsequent TestFlight and Apple
  gates are recorded below.

- **IMG-8160 TestFlight gate (2026-09-21 16:24 UTC):** Internal-only CD run
  `35625370999` was dispatched from exact source
  `837c0d9a26bcfc6a4990e0778664135408d792e4` with API origin
  `https://battery-chan-metro-liverpool.trycloudflare.com`, expected backend
  revision pinned to the same SHA, `upload_to_testflight=true`, and the
  owner-approved `test_environment_upload=true` fallback. The run completed
  successfully; its IPA provenance and Apple processing/status are recorded in
  the following entries.

- **IMG-8160 TestFlight upload (2026-09-21 16:32 UTC):** CD run
  `35625370999` completed successfully in 8m03s. It uploaded Build `0.1.0
  (72)` from exact source/backend `837c0d9a26bcfc6a4990e0778664135408d792e4`;
  the retained `release-provenance.json` records `uploadCompleted=true`,
  `uploadToTestflight=true`, and IPA SHA-256
  `011c2d7cb0204c326c461e946f92ddc8691b01a7d4cf041daa5618d7a46a4f8b`.
  Read-only Apple status workflow `35626544584` is now running for Build 72
  and the existing internal `Jason's friends` group; no tester or distribution
  mutation was requested.

- **IMG-8160 Apple closeout (2026-09-21 16:36 UTC):** Read-only status workflow
  `35626544584` confirmed Build `0.1.0 (72)`, id
  `1d15906e-5795-409b-8e05-a3a540d855a2`, is `VALID`, unexpired,
  `usesNonExemptEncryption=false`, arm64, and `IN_BETA_TESTING`; the Apple-
  processed bundle includes the iOS app and embedded Watch bundle. The
  existing internal `Jason's friends` group is `internal=true`, `allBuilds=true`,
  and includes Build 72. Apple still reports `internalReady=false`; this is
  retained as a physical-installation caveat, not an upload failure. No
  external distribution or tester mutation occurred. `PHONE-UX7` now moves to
  `evidence-open` for physical iPhone/Watch interaction only.

- **IMG-8160 candidate rebuild queued (2026-09-21 15:03 UTC):** After the
  fixture-identity correction and canonical-prefix decision fix, the next
  immutable candidate will use source `837c0d9a26bcfc6a4990e0778664135408d792e4`
  on loopback `127.0.0.1:39076`, container
  `aicaddie-release-837c0d9a-candidate-20260921`, and image tag
  `garmin-ai-caddie-api:837c0d9a26bcfc6a4990e0778664135408d792e4-candidate-20260921`.
  The existing `39074`/`39075` candidates and their tunnels remain reserved.
  Planned verification tunnel is tmux
  `codex-img8160-prefix-tunnel-20260921` on metrics port `20250`, expiring
  2026-09-22 15:10 UTC. Source CI `35615806302` is already running; no Native
  or TestFlight claim is made for `837c0d9a` yet.
...
- **PHONE-UX6 implementation checkpoint (2026-09-20):** The canonical prep/live/Watch
  ...
  prep/live maps no longer paint a club label over the course when the caddie strip
  already presents the next club. The ready-state banner was removed, first-hole
  loading remains bounded, and the iPhone/Watch picker uses Garmin's physical venue
  name. The earlier `第 N 个 9 洞组` presentation was a product-semantics error:
  Garmin's factual single-loop labels such as A/B/C and `老球场` must be preserved,
  with `场` appended only when the normalized label does not already end in `场`.
  Composite provider route tokens remain excluded. The start-round path no longer has a manual course-name
  write callback. User screenshots are resolved from `/home/ubuntu/IMG_*` before
  repository paths; the current evidence is `/home/ubuntu/IMG_8135.png` through
  `/home/ubuntu/IMG_8139.png`. Homeserver focused suite against the read-only source
  snapshot plus ephemeral writable data passed `323 tests, 2 skips`. The first Native
  Mobile CI attempt at `5681c6bd` failed only on the missing `CoursePrepStep.encode(to:)`
  synthesis and two Watch title assertions expecting no space after `只打`; both are
  corrected in commit `79e5ddca`. Exact-SHA Native run `35505364421` then reached the
  iOS compile gate and exposed one additional named-argument ordering error in
  `CourseReviewView` (`allowsRotation` must precede `showsPrepClubLabel`); that call is
  corrected in commit `5cca436b`. Exact-SHA Native `35506074392` then compiled the
  app but stopped at an existing test assertion that passed optional `routeOffsetM` to
  a non-optional accuracy overload; the test now unwraps that required field explicitly.
  Native `35506618561` then passed iOS target/Watch tests but the live flow failed because
...
- **CADDIE-P0 implementation (2026-09-19):** Code commit `27ece094` makes the
  ...
  full p10-p90 water and measured two-sided OB constraints, reprojects hazards
  after each shot, removes duplicate/dominated strategy cards, and returns an
  explicit no-route result instead of silently restoring an unsafe club. The
  final approach/recovery pass also applies hazards before club selection and
  prevents an infeasible option or unrelated sequence from remaining selected.
  Homeserver focused tests pass `194/194`; the expanded course-prep/mobile/
  readiness/geometry suite passes `275/275` with 2 conditional skips; changed
  files compile. Golden evidence has 12/12 passing cases, zero unchecked
  expectations and zero option/sequence mismatches at SHA-256
  `9b9c5833d256c20ab057238bedf1a337bd0cd18b9428d64a3122525d9808196c`.
  Warm per-hole decision p50/p95 remain below the 50/200 ms gates (worst
  `6.479/9.134 ms`). Source CI `35462771563` passed. Candidate
  `aa05695f` is healthy on `39072`; loopback/public revision, authenticated
  catalogue/localized names, a complete 18-hole package, and a real caddie
  decision passed through the fresh tunnel. Native run `35463569818` completed
  iOS/Watch builds, unit tests, live catalogue, 10/11 live UI tests, Watch
  runtime evidence and all artifact scans; the one failure was an old UI-test
  selector that matched the covered `live-caddie-entry` when the new explicit
  no-route result correctly supplied no recommended club. Correction `4d56cd56`
  adds a stable actual-club choice identifier and selects a real bag club when no
  recommendation exists; Source CI `35466407538` passed. Exact-SHA Native rerun
  `35466542942` then passed iOS unit tests,
  live catalogue preflight, 10/11 live UI tests, all Watch tests/runtime captures,
  artifact uploads and secret scans. Its only failure occurred later in the long
  real-app journey: XCTest delivered one tap to the visible full-catalogue action,
...
- **PHONE-UX5 course-name release (2026-09-15):** The iPhone, Apple Watch, and
  ...
  rename a Garmin venue. Source CI `35005531664`, exact-SHA Native Mobile CI
  `35005819951`, and internal TestFlight CD `35013065767` passed. Build 63 is
...
- **NET-PRIORITY network-lifecycle release (2026-09-16):** Commit
  ...
  7 skipped), lint, production build, and diff-check passed. Exact-SHA Native
  Mobile CI `35106347996` then passed all iOS/Watch build, test, live-catalogue,
  runtime-evidence, and secret-scan stages. The required Claude Opus 5
  read-only audit returned `GO` with no P0/P1 blockers; its raw JSON and report
  are retained under `/home/jason/garmin-ai-caddie-data/operations/`.
...
- **NET-PRIORITY internal release (2026-09-16):** The first CD attempt
  `35120211023` correctly stopped on degraded operational readiness after a
  successful archive/sign step. With the standing owner-approved internal-only
  fallback (`test_environment_upload=true`, `external_distribution=false`), CD
  `35121179159` uploaded Build `0.1.0 (64)` from exact source
  `fc5152ab77ef0566c66d5dda601a194b72fee55f`, embedding backend revision
  `41eb8e1ae237490b88757669bcde845640bb5e42` and candidate origin
  `https://response-breed-live-represent.trycloudflare.com`. Apple read-only
  check `35122318337` reported `VALID`, unexpired,
  `IN_BETA_TESTING`, `usesNonExemptEncryption=false`, and membership in the
  existing internal `Jason's friends` all-builds group. Exact IPA/Watch
...
- **PHONE-UX5 backend candidate (2026-09-14):** Revision
  ...
  `garmin-ai-caddie-api:3bbe79df0f645a4d9e1410c62110a3ad8991e7e9-candidate-20260914`
  (image id `sha256:7c7e5a8db48578b642807e416f21b4f2bb9f1513d5c010a9c43b453d4ea35979`).
  Public Quick Tunnel is `https://achievements-preparation-louis-major.trycloudflare.com`;
  health, topo-v11 and Native live preflight report the exact revision. Keep the
  candidate and tunnel for physical iPhone/Watch validation; no shared Funnel or
  production service was changed.
- **PERF-STARTUP backend candidate (2026-09-13):** Revision
  `70e74a6eb04bae1d656bc8cee81f534e0ae4f2b7` is healthy in container
  `aicaddie-release-70e74a6e-candidate-20260913` on loopback `39062`, image
  digest `sha256:d5f1cd2f877e90407b5959b4ffe8c5087d0153d8d2809ace0a1b24df046ffdfb`.
  Public Quick Tunnel is
  `https://family-heating-century-humanity.trycloudflare.com`; health,
  catalogue, topo and Native live preflight all report the exact revision.
  The candidate reuses the existing database/private volume and must remain
  available for physical validation; stable/rollback services are separate.
...
- **Native UX2 verification (2026-09-10):** Native Mobile CI run
  `34417237317` completed successfully at source `13474bf79638c086faf8b43aa1fa2044cdbb68a9`
  (product-code tip `b9ff8b9d`) against backend revision
  `5124d6384c71cba0e2f911ab043464aca3e28c46` and the public quick tunnel.
  All 39 Native Mobile CI steps passed: iOS/Watch compile and tests, live course
  preflight, real iOS XCUITest/screenshots/video, Watch runtime screenshots,
  evidence writing, and secret scans. The manifest records `dataMode=live` and
  `ios.status=watch.status=passed`. Artifact IDs/digests are
  `native-build-evidence` `10130995129` (`sha256:50f16b951ba1dfb797cf971a88bfc5f882267ba4b7b6a8b0ee3d739187a49266`),
  `real-screenshots` `10130695866` (`sha256:49a3ed5c692a36b4483ecf084b41c7039b51246186ad2880892a3f602ef427e9`),
  `real-video` `10130703039` (`sha256:274a4e460a628eaf0177a6ea47b8dccf939cc2cd530d010521228bf98caaf7e0`),
  `watch-real-screenshots` `10130994573` (`sha256:19682265b02e63f738fb5f68d0942ab0bab4c6990a0a66a8706ef9139825f16a`),
  `watch-snapshots` `10130826419` (`sha256:dd8298259febbd0849182f995394494e04a0d6c61a5aae5396c7853e98991da2`),
  and `design-snapshots` `10129769788` (`sha256:eb3ec91010f324c328ba466270dc26586769e693873aefc283f6724057e25d68`).
  The automated journey visibly contains one flag renderer, a next-club
  recommendation, and three selectable strategy cards; it does not by itself
  close physical-device proof, nor does it exercise the dedicated hazard detail
  page as a separate navigation assertion. No TestFlight or distribution side
  effect was started.
- **Native UX2 exact-SHA rerun (2026-09-10):** Native Mobile CI run
  `34527145107` completed at source
  `2910b4599ccd60a95414b2af6367ee7456e4de89`. The iOS app compiled and all
  309 iOS unit tests passed; SwiftJCS boundaries, design snapshots, live-course
  preflight, Watch tests/runtime captures, evidence writing, and every artifact
  secret scan also passed. The live iOS XCUITest step alone failed: both
  ...
  opened the caddie surface successfully, but their label-based
  `app.staticTexts["球童建议"]` query matched both the background entry label and
  the focused-sheet heading. The first raised “Multiple matching elements”; the
  second returned the non-hittable background match. This is an automation
  selector regression introduced by the new focused surface, not evidence that
  the caddie request failed. Artifact `native-build-evidence` `10174090228`
  (digest `sha256:3d69ffa624f2d015b28d05c11526e7b854ce1bce546b6a8a9e130178e0e409c2`)
  records `dataMode=live`, `ios.status=failed`, and `watch.status=passed`;
  `real-screenshots` is `10173466115`
  (`sha256:107adb17f31ce29b40df481883f127163a3e3fcf60618c7ad6e395f9080b3a4c`),
  `real-video` is `10173470160`
  (`sha256:427da275603be6209444905a52d6a762e7729baf87102f5133ca7143ad14a09c`),
  and `watch-real-screenshots` is `10174084475`
  (`sha256:830658cd2e3fcc1742cf3925bb3f8d03f3b1cf6852cb611ce2bbee34b1943d2f`).
  The failed journey stopped before its dedicated live hazard/caddie captures,
  so this run does not close simulator or physical-device evidence. The minimal
  fix assigns the sheet heading a stable `caddie-plan-heading` identifier and
  updates both UI-test helpers to use it; a fresh exact-SHA run is required.
- **Native UX2 exact-SHA rerun diagnosis (2026-09-11):** Native Mobile CI run
  `34534905003` completed at source `3b77b59601f21f28a6f9f1ebdc60d6833b468e31`.
  iOS compilation, all 309 iOS tests, SwiftJCS/design/live preflight, Watch
  build/runtime captures, evidence writing, and secret scans passed. The only
  failed step was the live iOS XCUITest capture:
  `TeeSelectionUITests.testNoCourseWithinFiftyKilometresStillOffersCompleteCatalogueFallback`
  found `course-catalog-result-31793` at the bottom edge (`y=848.33` on an
  852-point display). XCTest reported it hittable, but the tap point was under
  the home-indicator lane, so the search sheet stayed open and the expected
  `start-round-course-segment-31793` never appeared. This confirms a test
  viewport/gesture issue, not a catalogue or selection-state regression.
  The run's `real-screenshots` artifact is `10176463721`
  (`sha256:5acf1aae99af98ebbcceff74374bb51563ad925bfc8f7c1aa91bf6d90f569677`),
  `real-video` is `10176471381`
  (`sha256:a4b6147b9f24dcb5046c609d8767348ad4f5a0f06556bc80f0e73de20f8c44f7`),
  `watch-real-screenshots` is `10176788061`
  (`sha256:6dbb8b9fd9d8c7727acf0680f19d0dc6e04dcd90f24d896c58b3425fa5b51a3a`),
  and `native-build-evidence` is `10176788461`
  (`sha256:bf44e6238a21f53ced4bd4f13b3d06ed7b7b78394c22c95cbc9768c9fc0e4f08`).
  The same run's live hazard capture shows the legacy fallback as a thick red
  span with colliding `前/后` labels because deployed backend `5124d638` predates
  the `outlinePx` producer. The working tree now replaces that fallback with a
  compact red focus ring and separated edge tags, with focused geometry tests;
  the test viewport fix and renderer refinement are verified by the exact-SHA
  run recorded immediately below.
- **Native UX2 exact-SHA verification (2026-09-11):** Native Mobile CI run
  `34546023165` completed successfully at exact product source
  `04adc25cdb7a2bc30f3dff660cfe3869dd310caf`, against live API origin
  `https://suggests-kilometers-normal-insertion.trycloudflare.com` and backend
  revision `5124d6384c71cba0e2f911ab043464aca3e28c46`. The iOS app target passed
  311 tests plus 50 domain-package tests; the live XCUITest suite passed 11 tests
  (including all 7 TeeSelection tests, the dedicated hazard page, and caddie
  plan); the Watch target passed 329 tests. SwiftJCS boundaries, live preflight,
  design/runtime snapshot scans, evidence writing, and secret scans all passed.
  The manifest is `dataMode=live`, `ios.status=passed`, `watch.status=passed`.
  Artifacts: `native-build-evidence` `10180376265`
  (`sha256:ca67ec44448d93aeb379159a76ac1814406fc8a068ad546e1ef247886fbdedbc`),
  `real-screenshots` `10180158380`
  (`sha256:59396c5164097c8093b2c2a6131e6a6379ce20c61735b5b620af18ff4cffac60`),
  `real-video` `10180164097`
  (`sha256:fe7232d97051b2a34f81aa64628d3f2002081ee11079827fbd5fab21918c9c65`),
  `watch-real-screenshots` `10180375794`
  (`sha256:50b8ccb37cbd6f3b6d7c1319ae18e40caa63ed682aecdae95ff4c84df4e035ad`),
  `watch-snapshots` `10180256948`
  (`sha256:f813d3515ae959271d6d8e863d526ebabc0c2b78805e6be48fb5ab1ca9e86696`),
  and `design-snapshots` `10179280015`
  (`sha256:e0c24dfcd37cd29d7b41c1bb69676526d6ae5c49f534049eac69b966b62fa765`).
  Inspected live frames show one selected legacy hazard focus ring with separated
  front/back tags and 196/210-yard readouts, one primary caddie card
  (`一号木 216 码`, planned `一号木 → 六号铁`, expected 147-yard leave), and one
  rendered flag. This closes simulator evidence for the slice; physical iPhone/
  Watch installation, zoom panning, pole-foot drag, and fresh Garmin reconnect
  remain open. No TestFlight, deployment, external distribution, or data sync
  was started.
...
- **TestFlight build 51 (historical internal candidate):** iOS TestFlight CD run
...
- **TestFlight build 55 (current internal candidate):** The authenticated
  Phase 6 backend preflight first recorded a transient 20-second public
  readiness timeout; immediate direct local/public authenticated probes then
  returned in 6.45/6.62 seconds. The durable retry evidence reports
  `phone_reachable_backend_url=ready`, `backend_probe=ready`, authenticated
  readiness schema `ai-caddie-readiness-v1`, health schema
  `ai-caddie-health-v2`, and exact revision
  `5124d6384c71cba0e2f911ab043464aca3e28c46`. Its JSON is
  `/home/jason/garmin-ai-caddie-data/operations/backend-deploy-20260909-topo-v10/phase6-external-readiness-phone-ux2-before-internal-upload-retry1.json`
  with SHA-256
  `c46737ce48fb7491d0a2c878c2e5a3915be61fae5ad0b514f012c18149c42bd9`.
  iOS TestFlight CD run `34425296280` then built and uploaded `0.1.0 (55)`
  from source `aae339ba4bae8df9b8a7004f1d5f5064d116fa88` (product-code tip
  `b9ff8b9d`) with `test_environment_upload=true`,
  `upload_to_testflight=true`, and `external_distribution=false`. Fastlane
  reported Apple processing complete. Provenance binds the public quick-tunnel
  origin to backend revision `5124d638`, records `uploadRequested=true`,
  `uploadCompleted=true`, and `uploadToTestflight=true`, and gives IPA SHA-256
  `18b386a525a8d3c877c903bea94062ac456c07c42bdab227125ae0d2eb304b93`.
  GitHub artifact `AICaddie-ipa` ID `10132689779` has ZIP digest
  `sha256:6edc710f6bfd4a9e5e03ef391b5af1deb822c910bc26c871f8f12c8a538647f4`.
  A homeserver download independently matched the IPA and provenance fields,
  then its exact 11 MB temporary directory was removed. Read-only App Store
  Connect run `34426251893` reports build ID
  `41caf2d0-f158-4694-9f4d-3e7e7cf13056`, `VALID`, `expired=false`,
  `usesNonExemptEncryption=false`, `internalState=IN_BETA_TESTING`,
  `externalState=READY_FOR_BETA_SUBMISSION`, and processed arm64 app bundle
  `com.ai-caddie.mobile`. The existing internal all-builds group includes
  Build 55; external `Private Trial` does not. No Beta Review submission,
  external distribution, or production promotion was performed.
- **TestFlight build 54 (historical internal candidate):** iOS TestFlight CD run
...
- **Remaining release gate:** Current TestFlight build 55 is processed and
...
contract is present in the backend, iOS, and Web paths; the retired
`fast_start`/`first_hole_fast`/`fullCoursePending` protocol is absent from active
code and only negative assertions remain. The owner job-status route has an
explicit admin policy, member/owner tests use isolated durable stores and poll
terminal state, and iOS requires the new job fields instead of decoding the
retired terminal/409 payload. Source CI `35105911252`, exact-SHA Native Mobile
CI `35106347996`, the Opus 5 max/xhigh read-only audit, internal TestFlight
CD `35121179159`, Apple status check `35122318337`, and IPA diagnostic
...
**Owner decisions (2026-09-16, amended 2026-09-18):**
...
- 本轮新增一次 homeserver Claude Fable 5.1 `xhigh` 全项目只读审计；先审计、
  后决定网络实现，不以旧的局部 Opus/网络切片报告替代本轮结论。
...
7. 已完成：核实并统一 Garmin-authoritative canonical 球场名称协议，iPhone、Watch、Web 原样消费同一字段；收紧非 Garmin identity 门禁、复合路线拆分、普通 live package 的 snapshot 选择、iPhone 本地名称覆盖路径，并删除 Web/iPhone/Watch 的手填球场名入口。Source/Native 门禁、候选 TestFlight、Apple processing 和现有内部组可见性均已通过；实体设备名称与 Garmin 重连仍为 evidence-open。
...
- 公网 health、course options、tees、topo、Native live catalogue preflight 均通过并报告精确
  `70e74a6e` revision。Native Mobile CI `34759807648` 已全绿；随后内部 TestFlight
  CD `34763656027` 上传 Build 59，ASC 只读检查 `34764094689` 已确认 Apple processing
  和内部组可见性。
...
5. 已完成实现与静态验证：homeserver 聚焦测试 `296 passed, 2 skipped`、Python
   compileall、Source CI `34709596756`（exact `29d0a0c7`）和 Native
   `34709800015`（iOS/Watch、真实 simulator、evidence 全绿）均通过；内部
   TestFlight CD `34712702517` 正在运行；候选后端在
...
`Jason's friends` 组可见。后续 UX2 边界修正已在远端通过 374 个聚焦测试（5 个
跳过），Native Mobile CI `34417237317` 也已在 live 模式全绿；Build 53/54
保留为上一轮历史候选，不能作为本轮 follow-up 效果证据。当前 follow-up 已由
TestFlight CD `34425296280` 上传为 Build 55，Apple read-only 检查
`34426251893` 确认其已处理并在内部 all-builds 组可见。Build 55 随后的实体 iPhone
截图 `IMG_7915.png`、`IMG_7918.jpeg`、`IMG_7919.png` 未通过产品验收：障碍物同时
堆叠显示，难以辨认和选择；球童固定展示“推荐/保守/进攻”三套方案，即使路线相同
...
**Durable resume checkpoint (2026-09-12 02:06 UTC):** `PHONE-UX2` 的实现、完整 live Native
验证和内部 TestFlight 上传已完成，状态为 `evidence-open`。Build 55 已被最新实体 iPhone 反馈否决，
Build 57 已从精确源码 `70480f99751d1881db289806d6b3b623efdfa6bc` 上传并完成 Apple processing；不再等待旧版物理证据。障碍页应默认选中最近且仍相关的一个障碍；地图一次只强调该障碍，在有
几何边界时沿真实形状画红色轮廓，否则使用克制的红色前后沿指示；前沿/后沿距离是
主信息，并允许逐个切换。球童首页只显示一个主要推荐；只有杆序、落点或风险具有
实质差异时才在“其他选择”中显示替代方案，相同方案必须去重。推荐算法必须为球包中
每支实体球杆建立同一套独立表现模型，并用各杆自己的距离分布、方向/距离离散、失误
结果、样本量与场景适配度优化整条击球链及下一杆留距。A 杆比 P 杆稳定只是用户用于
说明原则的例子，不得针对 A/P 或任何杆名、杆种写特殊偏好。不得把“发球台必打一号木”
硬编码为规则，但普通无明显危险的四杆洞/五杆洞应由距离、离散度与风险自然选出 Driver。

**Durable execution plan (persisted 2026-09-10 19:19 UTC):**
1. 已完成：将 Build 55 的障碍堆叠和三卡重复问题收敛为独立障碍浏览页与单一主推荐；
   旧三卡/旧障碍文案契约已迁移到现行实现。
2. 已完成：iOS 障碍详情页采用单选模型，默认最近相关障碍；仅为选中障碍绘制红色
   轮廓/边界指示，突出前沿与后沿距离，并提供清晰的上一项/下一项或紧凑列表导航。
3. 已完成：后端与离线球童路线生成已为每支实体球杆用统一公式从其自身历史表现
   建模，并以距离分布、方向/距离离散、失误结果、样本置信度、场景适配和首选下一杆
   留距对整条击球链评分；不得按 A/P 等杆名特判。主推荐只显示一次，实质相同的候选
   去重，差异候选收进“其他选择”。
4. 已完成：更新拒绝旧三卡假设的 Swift/Python/UI 测试；homeserver 聚焦套件 `359
   passed, 2 skipped`（退出码 0）。最新 `LiveHazardDetailView.swift` 同时修复了畸形
   `outlinePx` 不应被插值到球路起点的问题；Native Mobile CI `34546023165` 已对该
   修正完成 iOS/Watch 与 Codable 边界验证并全绿。
...
6. 已完成修复：Native Mobile CI `34523500098` 已确认 iOS target 编译失败，原因是
   ...
   legacy route 标签回退。Swift 树同时已为 `CoursePrepHazardDetail` 增加畸形/旧格式
   `outlinePx` 的容错解码，并补充缺失、`null`、错误类型和畸形点数组回归测试；Native
   Mobile CI `34546023165` 已对最新 Swift/LiveHazard 修正完成 iOS/Watch 与 Codable
   边界验证并全绿。
7. 已完成诊断：精确 SHA Native Mobile CI `34527145107` 已在 `2910b459` 上通过 iOS
   编译、309 项 iOS 测试、Watch 和其余边界，但 live XCUITest 的两个 caddie helper
   因入口与弹层标题同名而匹配多个元素。当前只给弹层标题增加稳定 identifier，并迁移
   两个 helper；修复已提交为 `3b77b596`，同参数 live Native Mobile CI
...
8. 已完成：Native/source gates 通过后自动派发内部 TestFlight CD，使用
   `upload_to_testflight=true`、`test_environment_upload=true`、`external_distribution=false`；
   CD Run `34666136884` 已上传并处理 Build 57，ASC 只读核对 Run `34666574292` 确认
   `VALID`、未过期、`IN_BETA_TESTING`，并已在现有内部组可见。未做 Beta Review、外部发布、生产发布或数据同步；
...
当前 live 验证和 Build 55 的 API 入口为
...
`5124d6384c71cba0e2f911ab043464aca3e28c46`。该入口只在 Native/内部
...
| `REL` | `evidence-open` | Release and TestFlight gate; current internal build 63 is processed and group-visible. | Source/Native gates are green for product tip `d8769916`; CD run `35013065767` uploaded `0.1.0 (63)` from exact source `d8769916d4b7ea1fc8b2bdcfb151a67f944a6eb0` with candidate backend `7efd9853` and API origin pinned to `https://cuisine-collective-pulled-brass.trycloudflare.com`. Read-only ASC run `35014393530` confirmed `VALID`, unexpired, `IN_BETA_TESTING`, arm64, and inclusion in the existing internal `Jason's friends` group. Physical installation, fresh Garmin reconnect, and optional external Beta Review remain open. |
...
- `fc5152ab77ef0566c66d5dda601a194b72fee55f` / `41eb8e1ae237490b88757669bcde845640bb5e42`:
  ...
  Native Mobile CI `35106347996` passed; the latter used live catalogue
  preflight and completed iOS/Watch compile, tests, runtime evidence, and
  secret scans. The single authorized Claude Opus 5 `max` read-only audit
  returned `GO` with no P0/P1 blockers. Snapshot archive SHA-256 is
  `e9491c96cf556cf786b1e744850a474e44565f717210a2adbd7e50013b8445dc`; raw
  report SHA-256 is
  ...
  `35122318337` reported Build 64 `VALID`, unexpired, `IN_BETA_TESTING`, and
  visible in `Jason's friends`; exact artifact diagnostic `35122912635` passed
  iOS/Watch signing, profiles, bundles, and arm64 checks. No external
  distribution, production promotion, or tester mutation occurred. Physical
  iPhone/Watch interaction, GPS/name parity, and Garmin reconnect remain
  evidence-open.

- `d8769916` / `d8769916d4b7ea1fc8b2bdcfb151a67f944a6eb0`: PHONE-UX5
  course-name follow-up removed the user-facing manual course-name entry and
  made Garmin's canonical venue identity the only display authority across
  iPhone, Apple Watch, and Web. Source CI `35005531664` and exact-SHA live
  Native Mobile CI `35005819951` passed (`dataMode=live`, iOS/Watch status
  passed, runtime/evidence and secret scans passed). Native artifacts are
  `design-snapshots` `10412012327`, `real-screenshots` `10413197867`,
  `real-video` `10413895792`, `watch-snapshots` `10414331272`,
  `watch-real-screenshots` `10414620822`, and `native-build-evidence`
  `10414541180`. Internal TestFlight CD `35013065767` uploaded Build 63 with
  IPA SHA-256 `218cfcba97d0dc672b66cfad8379c9b81f624cc4d0a09ed6575cf6f9d9626321`
  and artifact `10415205447`. ASC read-only run `35014393530` confirmed Build
  63 id `5bb66ead-0b91-47db-877e-fe4372a491cf`, `VALID`, unexpired,
  `IN_BETA_TESTING`, arm64 app/Watch bundles, and inclusion in the existing
  internal `Jason's friends` all-builds group. No tester mutation, external
  distribution, production change, or data sync occurred.

- `65a6064f` / `65a6064f4061eb591bf5850f8b244e8725178904`: PHONE-UX5
  course-name authority remediation passed the targeted homeserver suite
  (`164/164`), Source CI `34900999198`, and exact-SHA live Native Mobile CI
  `34901057848` (`ios.status=watch.status=passed`, live catalogue/map journey,
  Watch runtime/evidence and secret scans). Native artifact IDs are
  `design-snapshots` `10370463232` (ZIP SHA-256
  `f42d8c828a6f8f72ae48ad5636a6a343ef0d44c94e0e5c3b1d93797b3311a26c`),
  `real-screenshots` `10372477394` (ZIP SHA-256
  `e90f7f7de8dafee3dfd3ee1a660f6452f340b2285fe1358a81c58b7ffef689c8`),
  `real-video` `10372247889` (ZIP SHA-256
  `f4be5bc4414175ddf7972462bc72da021fa6c5536089f334a912e61f6b254036`),
  `watch-snapshots` `10372342323` (ZIP SHA-256
  `9dffcfb16a9455af4460c679f5568efceb322ffd13a98cd3a0aad046c4706869`),
  `watch-real-screenshots` `10373140051` (ZIP SHA-256
  `ebcef1c1eb517492916c4f207a7b4860478325eb8e9cf002031312bb433f36a5`),
  and `native-build-evidence` `10372574612` (ZIP SHA-256
  `95dfb048ada179f149410c339ce0f7d14f197a51fb9c162f7f85dbc8cea324f7`).
  The internal TestFlight CD `34906576121` uploaded `0.1.0 (62)`; release
  provenance records IPA SHA-256
  `165f6ba7c3b2875447f28b55ee07c0a6b50ae91bbf201a554a6c68cdbf8830c6` and
  `AICaddie-ipa` artifact `10372912602` (ZIP SHA-256
  `58314d4261b986cfb93f8e1315df9814da3cf3a3f2ba229425edf634cdb30658`).
  ASC read-only run `34907384887` confirmed Build 62 id
  `31f05d8b-3219-40c6-b52d-2989228aa357`, `VALID`, unexpired,
  `IN_BETA_TESTING`, arm64 app/Watch bundles, and inclusion in the existing
  internal `Jason's friends` all-builds group. External `Private Trial`, Beta
  Review, production, and data sync were unchanged.
...
- `d41fed9e` / `d41fed9ef65774ef0dceb430b3597ae0f040c70b`: PHONE-UX5 focused
  ...
  diff checks passed. Source CI `34769988882` and exact-SHA live Native Mobile
  CI `34770331525` passed with `dataMode=live`; iOS/Watch tests, live iOS
  journeys, Watch round seed/restore screenshots, evidence writing, and all
  secret scans are green. Artifacts are `design-snapshots` `10321344310`
  (`sha256:1074aa15558144b66276e92e4f7ea98b8bdecaae0a2579d0270d759ea607eaa1`),
  `real-screenshots` `10322816326`
  (`sha256:e6e5e5399a957645a4b580ee57b45396501c1050748a11e547d02d3358b0d3bc`),
  `real-video` `10323025946`
  (`sha256:55521c89f3f9ba5f76162cc40bf16f6ede268c9f00a345210c5902f107b30b5a`),
  `watch-snapshots` `10322652102`
  (`sha256:f83ad725c4dce1d37cdf36f6123678b49c1d7c8909817b664b597835bffb1449`),
  `watch-real-screenshots` `10323091561`
  (`sha256:72bd7afb3a099cc684f5c838cfd02bd1f99ff7b55fb2d7543c5dc7f90e7321df`),
  and `native-build-evidence` `10322926984`
  (`sha256:6b136095e3b869603f3fccee7148e3a9cb016545efd57de1c732795203ba2d5d`).
  Remote screenshot review confirms Chinese-first course labels, factual map
  before caddie computation, one primary tee recommendation, one selected
  hazard with fixed-size edge labels, and a single rendered flag. The remaining
  proof is physical zoom/drag and Garmin reconnect; no all-course offline map
  storage was added.
- `d41fed9e`: TestFlight CD `34774424885` uploaded internal-only `0.1.0 (60)`
  with `upload_to_testflight=true`, `test_environment_upload=true`, and
  `external_distribution=false`. Release provenance binds source and backend
  to `d41fed9ef65774ef0dceb430b3597ae0f040c70b`, API origin
  `https://changed-carb-sampling-push.trycloudflare.com`, and IPA SHA-256
  `2ee407533c30aabfe2a42fa52d2256aed508193461460f8a9ac1216081765818`;
  `AICaddie-ipa` artifact `10323251014` has ZIP digest
  `sha256:69656938cc6e21a15e6a099c5140a1225abcbf7500f8564add4d54cdfde2c88b`.
  Read-only ASC run `34774892396` confirmed Build 60 id
  `5aeaf8be-a6be-4fbf-8533-9fd81707a4cf`, `VALID`, unexpired,
  `IN_BETA_TESTING`, `usesNonExemptEncryption=false`, arm64, and inclusion in
  the existing internal `Jason's friends` group. External `Private Trial`,
  Beta Review, production, and data sync were unchanged.
- `70e74a6e` / `70e74a6eb04bae1d656bc8cee81f534e0ae4f2b7`: Source CI
  `34757661927` and exact-SHA live Native Mobile CI `34759807648` passed. The
  Native manifest records `dataMode=live`, `source=production`, and
  `ios.status=watch.status=passed`; live preflight, iOS tests/XCUITest, Watch
  tests/runtime captures, evidence writing and all secret scans passed.
  Artifacts are `design-snapshots` `10318453216`
  (`sha256:b15a515a5d0c3b32df06c76bd907f5fb242b0707b1409a8ec48d2df28e4d5220`),
  `real-screenshots` `10319900575`
  (`sha256:06b11444afe0980596852ebb96fb968e2d8dbf436ec18485d60efd7e3548c275`),
  `real-video` `10319746047`
  (`sha256:07a1acf4883402ad7c840934297359364ab2a858c8713637fb4dd413feb9e9ec`),
  `watch-snapshots` `10319087845`
  (`sha256:3a63eb65ca30c6da7d4e84bcc4c32b1744c6d3d21a2b549130a6c0ac380f06e9`),
  `watch-real-screenshots` `10319078343`
  (`sha256:fa88016af84bd664452031fe6a97fde89487af0e6da56ea41919fb43efbdcbfc`),
  and `native-build-evidence` `10319013580`
  (`sha256:a80c2146d6648f37c37183b9c014ec008e4478799eab783b0889f4d07e7a6a4c`).
- `70e74a6e`: TestFlight CD `34763656027` uploaded internal-only `0.1.0
  (59)` with `upload_to_testflight=true`, `test_environment_upload=true`, and
  no external distribution. Release provenance binds both commit and backend
  revision to `70e74a6eb04bae1d656bc8cee81f534e0ae4f2b7` and API origin
  `https://family-heating-century-humanity.trycloudflare.com`; IPA SHA-256 is
  `5ea5199c0ca6941f42337c90947be976a1c98f38affb2e488e30c61c6e8207f1`.
  `AICaddie-ipa` artifact `10318879880` has ZIP digest
  `sha256:67e9ef9b6c1bd87f235273f3f5f1df82d4e0a1b1b51195eec94ef9a2b91cadd1`.
  Read-only ASC run `34764094689` confirmed Build 59 id
  `77c928b7-f1c3-40e2-95ea-116d6094e298`, `VALID`, unexpired,
  `IN_BETA_TESTING`, `usesNonExemptEncryption=false`, arm64, and included in
  the existing all-builds internal `Jason's friends` group. External
...
- `70480f99` / `70480f99751d1881db289806d6b3b623efdfa6bc`: Source CI
  ...
  preflight, SwiftJCS/design checks, real iOS screenshots/video, dedicated
  hazard and caddie journeys, Watch runtime screenshots, evidence writing, and
  all secret scans in `dataMode=live`. Artifacts are `design-snapshots`
  `10287813975` (`sha256:4c0f68620e795b9547df01513f25bc044eca9d2ec0f818528e7b088df56d5b86`),
  `real-screenshots` `10289500775`
  (`sha256:1cd0ccd418dcef2eb334fb37138d3b3f126153d928f30b033b24a990102a59cb`),
  `real-video` `10289480804`
  (`sha256:0b747c94b5fd0aab0672be4fc55c9e7fe811b14e71a8b909ad8034b23bdd126d`),
  `watch-snapshots` `10289371230`
  (`sha256:18449c1539dab4065b2a11d5e18ea6564b831e66bb1113682e5ede25fc5c378e`),
  `watch-real-screenshots` `10288847095`
  (`sha256:c292015c3372d657a22833fb8efc84b011fc8b7e779124d7f2eac643ce7c577f`),
  and `native-build-evidence` `10288837053`
  (`sha256:39a97ac79e09b5e1923fff169c3a839ce728aa31655a4660a3d50317609bbb3d`).
  The live run used API origin
  `https://talented-claims-paris-august.trycloudflare.com` and backend revision
  `c82c0547277c83244ed02f2c4f07275019c7a6c8`.
- `70480f99`: TestFlight CD `34666136884` built and uploaded internal-only
  `0.1.0 (57)` with `upload_to_testflight=true`,
  `test_environment_upload=true`, and `external_distribution=false`. Release
  provenance binds the same API origin/backend revision; IPA SHA-256 is
  `27aaecd5e98766542b3e5bfcc8b23bcec020469b6ad5563c0486d5f319c608b4` and
  `AICaddie-ipa` artifact ID `10288772688` was finalized after Apple accepted
  the binary. Fastlane reported processing complete for Build 57. Read-only
  ASC Run `34666574292` confirmed `0.1.0 (57)` `VALID`, unexpired,
  `IN_BETA_TESTING`, `usesNonExemptEncryption=false`, arm64, and visible in the
  existing internal `Jason's friends` group; external `Private Trial`, Beta
  Review, production, and data sync were unchanged.
...
- `13474bf7`/`b9ff8b9d`: Native Mobile CI `34417237317` passed all required
  iOS/Watch steps in `dataMode=live` against backend `5124d638`. The run
  uploaded real iOS screenshots/video, Watch runtime screenshots, design
  snapshots, and the sanitized native evidence manifest; all artifact secret
  scans passed. Artifact IDs and digests are recorded in the Native UX2
  verification entry above. This evidence is simulator/CI evidence only; no
  TestFlight or external distribution side effect was performed.
...
- 2026-09-19: CADDIE-P0 Source CI `35462771563`, exact-revision candidate
  ...
  live iOS tests but failed because an August UI selector matched the covered
  `live-caddie-entry` when hole 2 correctly had no recommended club. The bounded
  working-tree fix identifies actual-club buttons explicitly and falls back to a
  real bag club without fabricating a caddie recommendation; no TestFlight was
  dispatched from the failed gate.
...
- 2026-09-14: Exact-SHA Native Mobile CI `34901057848` completed successfully
  ...
  real iOS and Watch runtime captures, evidence writing, and all artifact
  secret scans green. Artifacts: `design-snapshots` `10370463232`,
  `real-screenshots` `10372477394`, `real-video` `10372247889`,
  `watch-snapshots` `10372342323`, `watch-real-screenshots` `10373140051`,
  and `native-build-evidence` `10372574612`. The manifest records live data
  with both iOS and Watch passed.

- 2026-09-14: After the source/Native gates passed, internal TestFlight CD
  `34906576121` built and uploaded `0.1.0 (62)` from exact source `65a6064f`
  against backend `3bbe79df`; `external_distribution=false` and no production
...
- 2026-09-14: Native Mobile CI `34889718192` at `3c4a6098` compiled the iOS
  target and passed the Watch target, but `StartRoundDiscoveryTests` caught a
  real reconciliation regression: old Garmin played-route names (`A/B`,
  `B/C`, `C/A`) no longer recovered their individual 9-hole labels. The
  current fix only derives the first loop code when a factual 9-hole catalogue
  or downloaded structure is present; composite route names remain rejected as
  standalone selectable rows. Fresh Native CI is the next gate.

- 2026-09-14: Native Mobile CI `34891758376` confirmed the recovery branch was
  reached but its character validation still included `/` and `+`, so `A/B`,
  `B/C`, and `C/A` remained unlabeled. The parser now removes those separators
  before validating loop letters; no new name source or alias was introduced.
...
- 2026-09-13: `PHONE-UX5` moved from `in-progress` to `evidence-open` after
  ...
  TestFlight CD `34774424885`, and ASC read-only check `34774892396` all passed.
  Build 60 is `VALID`/`IN_BETA_TESTING` and visible in the existing internal
  group. Native screenshots were reviewed for the reported map/hazard/caddie
  regressions; physical zoom/drag and Garmin reconnect remain open. No external
  distribution, production promotion, or data sync occurred.

- 2026-09-14: `PHONE-UX5` reopened as the sole `in-progress` slice for IMG_8050.
...
- 2026-09-14: Targeted homeserver verification ran the six affected backend
  modules (`164/164` tests passed) against the current source. Commit `e46a866a`
  updates `RealFlowUITests` so a partial map is required to show the preparation
  disclosure only while it is still partial; a concurrent ready transition is
  accepted and the precise-map assertions continue. The next action is a fresh
  exact-SHA Native Mobile CI run, followed by the already-authorized internal
  TestFlight upload and Apple processing check.
...
- 2026-09-13: `PERF-STARTUP` moved from `in-progress` to `evidence-open`.
  ...
  `34759807648` passed at `70e74a6eb04bae1d656bc8cee81f534e0ae4f2b7`;
  the Native manifest is `dataMode=live` with iOS and Watch both `passed`.
  Internal TestFlight CD `34763656027` uploaded Build 59 against the exact
  `70e74a6e` candidate, and ASC list run `34764094689` confirmed `VALID`,
  unexpired, `IN_BETA_TESTING`, arm64 and internal-group membership. Physical
  iPhone/Watch startup and interaction evidence remains open; no production,
  external distribution or data sync occurred.

- 2026-09-13: TestFlight CD `34763157590` failed before upload because the
  manually typed expected backend revision omitted its final hex character.
  Archive/signing succeeded, but the 40-character provenance gate correctly
...
- 2026-09-13: Implemented the approved startup optimization set in the canonical
  ...
  2 skipped`); Source/Native/TestFlight gates are now complete for the candidate,
  while physical device interaction and Garmin reconnect remain evidence-open.
...
- 2026-09-12: Native Mobile CI `34709800015` completed successfully at exact
  ...
  live simulator journeys, hazard/green/caddie captures, Watch runtime
  screenshots, evidence writing, and secret scans passed. Per the standing
  owner rule, internal TestFlight CD `34712702517` was dispatched immediately
  with the candidate Quick Tunnel and backend revision
  `17ad4e66871126730c9fca9246fee884af309b2f`; external distribution remains off.

- 2026-09-12: Native Mobile CI `34709800015` is running at exact source
  `29d0a0c7a3fe8df73d7466e3765a60596996b03d`. Launch prerequisites, live API
  resolution, and environment validation passed; the iOS app target is in
  progress. This run includes the multi-hole fast-start fixture correction.
  The next action remains automatic internal TestFlight upload only after all
  Native iOS/Watch and evidence stages pass.
...
- 2026-09-12: Native Mobile CI `34706816330` reached all cleanup and Watch
  evidence stages but failed the iOS target at the single compiler error
  `AICaddieApp.swift:1285: 'return' cannot transfer control out of a defer
  statement`. No simulator/live product assertion ran and no release action
  followed. Commit `f3b04dd853e2508cf48f6a4f5bcb6c30ecc61e05` replaces that
  defer guard with a conditional assignment while preserving generation
  ownership; the next exact-SHA Native run must use this commit.
...
- 2026-09-11: Native Mobile CI `34534905003` completed at exact source
  ...
  Watch, evidence, and secret gates passed; only the live catalogue fallback
  journey failed. The result row was reported hittable at `y=848.33`, but its
  tap point was underneath the 852-point home-indicator lane, leaving the
  search sheet open. The failure is therefore test geometry, not product
  selection state. The run's dedicated hazard screenshot also confirmed that
  the deployed backend lacks `outlinePx`, making the legacy red span and `前/后`
  labels collide. The working tree now adds a full-safe-viewport tap helper and
...
- 2026-09-11: Exact-SHA Native Mobile CI `34546023165` completed successfully
  at `04adc25c`. The iOS app target passed 311 tests plus 50 domain-package
  tests; the live iOS suite passed 11 tests (including the catalogue fallback,
  dedicated hazard page and caddie plan); the Watch target passed 329 tests.
  SwiftJCS, live preflight, design/runtime/evidence scans and secret scans all
  passed. The six artifact IDs/digests and the inspected `10b-live-hazard`,
  `11-caddie-plan`, and `10-live-hole` frames are recorded in the verification
  entry above. This closes simulator evidence but not physical iPhone/Watch
  validation. No TestFlight, deployment, external distribution, or data sync
  was started.

- 2026-09-11: Closed the allow-listed evidence scratch created for Native CI
  `34546023165`. Remote source/evidence paths
  `/home/jason/codex-runs/garmin-ai-caddie-phone-ux2-20260911-a` and
  `/home/jason/codex-runs/garmin-ai-caddie-phone-ux2-20260911-b`, plus the local
  `/tmp/aicaddie-phone-ux2-view-4UFAs7` visual copy and marker, were removed only
  after open-handle checks. GitHub artifacts remain retained. The persistent
  cleanup record, artifact digests, pre/post sizes, and manifest hashes are in
  `/home/jason/garmin-ai-caddie-data/cleanup-manifests/20260911T010627Z-phone-ux2-evidence/`;
  no containers, ports, tunnels, source data, or credentials changed.
...
- 2026-09-10: Native Mobile CI `34527145107` completed against exact source
  ...
  failed because two UI helpers queried the duplicated visible label “球童建议”:
  the entry and focused-sheet heading both matched, producing one multiple-match
  error and one non-hittable background match. The manifest correctly records
  `dataMode=live`, `ios.failed`, and `watch.passed`. Artifacts were inspected in
  `/home/jason/codex-runs/garmin-native-34527145107` (created by this Codex
  ...
  exact-SHA rerun `34546023165` completed successfully; its final evidence is
  recorded above.
...
- 2026-09-10: `CoursePrepHazardDetail` now treats `outlinePx` as an optional
  ...
  values fail closed to an empty outline instead of invalidating the enclosing
  hole package; valid three-or-more two-dimensional finite-point outlines remain
  available to the selected hazard renderer. Added Swift regression coverage for
  each boundary. The homeserver focused suite on the synchronized source copy
  passed `292 tests, 2 skips` (exit 0); a fresh Native Mobile CI on the resulting
  commit is still required.

- 2026-09-10: Full homeserver discovery was rerun with the source snapshot's
  `.git` and `.env.example` present, Git authority tooling available in the
  ephemeral container, and the test process running as the snapshot owner so
  unreadable-file cases remain meaningful. Result: `Ran 2072 tests in 64.789s;
  OK (skipped=13)`, exit 0. The command used the API image's existing
  `/app/.venv`, a temporary `git` package layer, `PYTHONPATH` for its existing
  `pathspec` module, and no ports, tunnels, volumes, or persistent dependency
  installation. The final log is
  `/home/jason/codex-runs/garmin-ai-caddie-phone-ux2-20260910-e/full-test-output-final-pass2.log`;
  it remains allow-listed until evidence is archived and cleanup is recorded.

- 2026-09-10: Durable `PHONE-UX2` checkpoint corrected the focused homeserver
  evidence to `359 passed, 2 skipped` (exit 0) and recorded the latest
  `LiveHazardDetailView` malformed-`outlinePx` guard. A full discovery run was
  attempted in an environment missing Git metadata and `.env.example`; after
  those inputs were restored, two failures still reflect superseded contracts:
  `tests/test_server_v2_caddie.py:110` hard-codes the old three sequence IDs,
  while `tests/test_server_v2_readiness.py:663-665` hard-codes the old
  `14 medium / 4 low` confidence split. The next action is to inspect the new
  algorithm's actual fixture output and update the contracts or model, then
  rerun discovery with Git and `.env.example` present.

- 2026-09-10: The authenticated Phase 6 backend preflight recorded one
  transient 20-second public readiness timeout, followed by local/public
  authenticated responses in 6.45/6.62 seconds and a successful persisted
  retry (`backend_probe=ready`, exact backend `5124d638`). TestFlight CD run
  `34425296280` then built and uploaded internal-only `0.1.0 (55)` from source
  `aae339ba`/product tip `b9ff8b9d`; Apple completed processing. IPA SHA-256 is
  `18b386a525a8d3c877c903bea94062ac456c07c42bdab227125ae0d2eb304b93`;
  artifact `10132689779` has ZIP digest
  `sha256:6edc710f6bfd4a9e5e03ef391b5af1deb822c910bc26c871f8f12c8a538647f4`.
  Read-only ASC run `34426251893` confirmed Build 55 `VALID`, unexpired,
  `IN_BETA_TESTING`, arm64, and included in the existing internal all-builds
  group; `Private Trial` does not include it. The exact 11 MB homeserver
  artifact-verification directory was removed. No external distribution, Beta
  Review, production change, or data sync was performed. `PHONE-UX2` remains
  `evidence-open`; the sole next action is physical iPhone/Watch validation of
  map panning, the dedicated hazard page, pole-foot flag dragging, and all
  three caddie strategies on Build 55.
...
- 2026-09-09: Fresh Native Mobile CI `34407853548` was dispatched from
  ...
  backend revision `6d130528`. It later completed with the three iOS topo
  waits failing on the known `topo-v9`/`topo-v10` deployment mismatch; no
  TestFlight, signing, or distribution side effect was started. This is a
  historical diagnosis superseded by run `34417237317` below.

- 2026-09-10: Native Mobile CI `34417237317` completed successfully at source
  `13474bf7` (product-code tip `b9ff8b9d`) against backend `5124d638`.
  All 39 required steps passed in `dataMode=live`, including iOS/Watch build
  and tests, live preflight, real iOS screenshots/video, Watch runtime
  screenshots, evidence writing, and secret scans. Artifact IDs/digests are
  recorded in the Native UX2 verification entry. The exact temporary inspection
  directory `/home/jason/codex-runs/garmin-ai-caddie-phone-ux2-native-20260910-a`
  was created with a 24-hour manifest, inspected on homeserver, and cleaned up
  after the artifact checks. `PHONE-UX2` therefore moved from `in-progress` to `evidence-open`;
  physical iPhone/Watch and dedicated hazard-page proof remain open. No
  TestFlight, signing, or distribution side effect was started.

- 2026-09-10: `PHONE-UX2` follow-up implementation was rechecked after context
  recovery. The homeserver-only command
  `docker run --rm ... /app/.venv/bin/python -m unittest
  tests.test_decision_layer tests.test_course_prep tests.test_mobile_contracts
  tests.test_server_v2_mobile` ran `292` tests with `2` skips and exited `0`.
  The six initial failures were resolved without changing the decision schema:
  risk-aware safe/recovery fallback is restored for materially dangerous or
  obstructed situations; long-hole assertions read `selectedSequence.id`; club
  alias assertions validate canonical physical identity; and hazard-distance
  assertions live on `LiveHazardDetailView`, not the caddie page. The temporary
  source snapshot was `/home/jason/codex-runs/garmin-ai-caddie-phone-ux2-20260910-c`;
  its container was `--rm`, with no ports, tunnels, volumes, or dependency
  installs. It remains allow-listed only until the final cleanup check below.
...
- 2026-09-09: Native Mobile CI `34406253005` reached all Watch build/runtime evidence
  ...
  the math helper public. A fresh Native run against `b9ff8b9d` was required;
  that requirement was satisfied by run `34417237317`; no release side effect
  was performed by this failed run.
...
- 2026-09-09: Context recovery confirmed the sole active slice remains
  ...
  `a44f1c9f` entry was stale). The then-next action was a fresh Native Mobile
  CI run against `b9ff8b9d`; run `34417237317` subsequently satisfied that
  gate. No TestFlight or other release side effect was authorized by that
  recovery step.
...
- 2026-09-09: `PHONE-UX2` focused verification was rerun in the documented
  ...
  persistent homeserver data was changed. At that checkpoint native/device
  evidence remained the next gate; Native CI run `34417237317` has since
  closed its simulator/CI portion, while physical-device evidence remains open.
...
- 2026-09-09: `PHONE-UX2` follow-up implementation passed the focused remote
  ...
  native build/device run was subsequently completed by `34417237317`; no
  release or deployment side effect was performed. Test scratch
...
- 2026-09-12: Current `PHONE-UX2` follow-up product tip is
  `70480f99751d1881db289806d6b3b623efdfa6bc`. Source CI `34663338160` passed
  after the bounded accessibility containment fix. Exact-SHA full live Native
  Mobile CI `34663501590` passed against API origin
  `https://talented-claims-paris-august.trycloudflare.com` and backend revision
  `c82c0547277c83244ed02f2c4f07275019c7a6c8`, with `capture_scope=full`,
  `fixture_mode=false`, and `require_live_preflight=true`. It completed the
  real iOS UI journey and all Watch/evidence stages; no Native failure remains
  for this tip.
- 2026-09-12: Native run `34659950957` first isolated the green-entry
  accessibility failure at `TeeSelectionUITests.swift:404`; the entry existed
  visually but the hero exposed one merged `live-open-map-from-hero` element.
  Commit `70480f99` adds `.accessibilityElement(children: .contain)` so map and
  green controls remain separate. Exact-SHA rerun `34663501590` passed iOS
  (313 tests and 11 live UI tests), Watch (329 tests), live preflight, runtime
  evidence, and all artifact secret scans. The follow-up internal TestFlight CD
  `34666136884` uploaded Build 57 and read-only ASC check `34666574292`
  confirmed it `VALID`, unexpired, `IN_BETA_TESTING`, and visible in the
  existing internal group. The remaining gate is physical iPhone/Watch
  installation and interaction evidence; no external distribution or
  production promotion was performed.
- 2026-09-15: Commit `d8769916` updates the live-course UI regression assertion
  to accept the explicit `topo-hole-base-loading` transition between partial
  CourseView vectors and the decoded precise topo; it still requires the
  `live-map-preparing` disclosure when the map is genuinely partial. Source CI
  `35005531664` passed backend, frontend, and Docker jobs. Exact-SHA Native
  Mobile CI `35005819951` passed against API revision `7efd9853` and the
  candidate Quick Tunnel. Internal TestFlight CD `35013065767` uploaded Build
  63; ASC read-only check `35014393530` confirmed `VALID`,
...
- 2026-09-19: `CADDIE-P0` code commit `27ece094` completed the 12-case blocking
  production replay and hard-feasibility/stability planner. It prohibits Driver
  after the tee, applies full carry distributions to water, measured lateral
  dispersion to two-sided OB corridors, reprojects every continuation, removes
  dominated strategy cards, and exposes explicit infeasible results. The final
  review closed approach/recovery hazard filtering and selected-option/sequence
  alignment. Homeserver focused tests passed `194/194`; the expanded suite
  passed `275/275` with 2 skips; changed-file compilation passed. Golden evidence
  remains deterministic at SHA-256
  `9b9c5833d256c20ab057238bedf1a337bd0cd18b9428d64a3122525d9808196c`
  with 12 cases, no failed/unchecked checks and no selection mismatch. Warm
  per-hole decision worst p50/p95 was `6.479/9.134 ms`. Source/Native/backend/
  TestFlight/Apple gates are next; no production or external-distribution side
  effect has occurred.
- 2026-09-19: The actual-club selector correction was committed as `4d56cd56`
  and Source CI `35466407538` passed. Exact-SHA Native rerun `35466542942`
  completed iOS unit tests, live catalogue preflight, 10/11 live UI tests,
  Watch tests/runtime captures, all artifact uploads and all secret scans. Its
  sole failure was `RealFlowUITests.swift:1015`: after a 474-second live journey,
  XCTest synthesized one tap on the visible `start-round-search-all-courses`
  button but the sheet did not present. The retained screenshot and accessibility
  tree show the unobstructed start screen; all seven later TeeSelection tests
