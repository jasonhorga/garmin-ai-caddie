...
- **PHONE-UX6 implementation checkpoint (2026-09-20):** The canonical prep/live/Watch
  ...
  corrected in the working tree and require a fresh focused run, commit, and exact-SHA
  Native rerun before any TestFlight claim.
...
| `PHONE-UX6` | `in-progress` | Implement the `IMG_8135`-`IMG_8139` slice: canonical structured prep/live caddie plans with all landing legs, selectable plan-map state, removal of the offline-ready banner, coherent local-first round loading, and iPhone/Watch identity/interaction parity. Keep download-speed diagnosis measured and separate from UI claims. | Source commit `5681c6bd` passed the remote focused suite (`323 tests, 2 skips`). Native attempt at that SHA exposed two correctness gates; `CoursePrepStep.encode(to:)` and Watch `只打` spacing are now fixed in the working tree. Next evidence is a new source commit, exact-SHA Native Mobile CI, then the automatic internal TestFlight path. Screenshots live under `/home/ubuntu/IMG_*`. |
