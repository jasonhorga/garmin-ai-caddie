...
compatibility consequences. Revisit that migration only after the Cloud audit
and the open physical release gates are closed.
...
| Current tip | `integration/v2` points to `f0b193a2`, which only records the Cloud audit start after the reconciliation. |
...
| `evidence/*` | 27 | 0 | 27 | Preserve until the Cloud report and artifact hash are archived; never merge as product code. |
...
to delete them during the Cloud audit.
...
* A `reconcile/<date>` branch is a one-time ancestry operation. It gets an
  expiry date and is closed after the target merge and audit handoff.
...
1. Keep `integration/v2@f0b193a2` as the single canonical line.
...
3. Keep old refs, PRs, release tags, and evidence refs through the Cloud audit.
...
After the Cloud report and hash are archived, cleanup may proceed in a separate
