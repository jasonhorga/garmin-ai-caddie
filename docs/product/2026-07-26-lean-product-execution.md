...
| E2E-01 | `RUNTIME COMPLETE / USER VISUAL GATE OPEN` | [Native run 30507566447](https://github.com/jasonhorga/garmin-ai-caddie/actions/runs/30507566447) 已在同一北京丽宫 local round 完成 iPhone 1–18 洞、强杀恢复、历史修正、在线球童、最终 ACK/finish 与回首页；最终 [Watch run 30515480491](https://github.com/jasonhorga/garmin-ai-caddie/actions/runs/30515480491) 已在同一 Cypress Point round 完成 1–18 洞、18/18 真实地图、每洞唯一首杆 GPS、强杀恢复、历史修正、56 条事件全部 ACK、远端 finish、本地 round 清除、真实缓存坐标附近分组与无参数重启回 `WatchStartView`。实现门已关闭；剩余工作仅为把冻结批准图和最终模拟器图并排交用户审批，审批前不发布 TestFlight。 |
...
| E2E-03 | `FIX READY / 待隔离凭证运行验证` | 只读生产审计确认 owner 历史当前 `461` 轮，其中 2026-07-30 的 `5` 轮 Cypress Point 均为 `55/75`、每轮对应唯一 `watch-*` round 且恰有 `56` 条事件；根因是 Watch runtime 用 owner admin token 完成真实 `/finish`。当前 RED→GREEN 配置测试要求 workflow 只读 `AI_CADDIE_CI_PLAYER_TOKEN`，Watch 测试入口以 Bearer 写入独立 player 分区且不注入 admin token；Watch 完成后，同一隔离 player 的真实球局会由后继 Web job 生成复盘工作台、球局列表和球局详情三张 PNG，trace、自动失败截图、HTML report 与 admin token 均不进入 artifact。尚未删除或修改这 5 轮；创建 CI player、写入 GitHub secret 后才允许再次跑真实旅程。 |
...
| 4 | CourseView Deep Mine 将现有 release/prodgeometry 中未使用的真实数据逐项转成产品判断 | `IN PROGRESS — prodgeometry 全资产已闭合；其余账本继续` | 对未分类字段/mesh 给出可重放证据、跨球场语义和明确的使用/不使用结论，不为挖数据而增加无产品价值的平台；会员 Green Contours 抓包等用户方便时补，不阻塞其他分支 |
