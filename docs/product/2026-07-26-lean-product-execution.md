...
当前执行指针：仓库内的软件任务已经关闭，不再从地图或视觉细节派生新开发。下一步只可能由以下外部门之一触发：为 E2E-03 创建隔离 CI player 并配置 `AI_CADDIE_CI_PLAYER_TOKEN`；获得发布授权后用物理 Apple Watch 验证 AutoShot 误报/漏报、后台、续航与发热；用户方便时提供会员 Green Contours 正反球场抓包。三者彼此独立，均不得改用 owner token、模拟器或普通 DSKIMG 冒充证据；在任一门具备前，当前候选保持冻结，不发布 TestFlight。
