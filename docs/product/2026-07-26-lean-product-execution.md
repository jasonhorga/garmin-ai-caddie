...
## 里程碑 2 当前增量（2026-07-26）

- 产品代码 SHA：`b37d5cf8a40261678262547606dbb9f84fe0b0a4`。
- Native CI：[run 30192900947](https://github.com/jasonhorga/garmin-ai-caddie/actions/runs/30192900947) 整体成功：Domain `47/47`、iOS `110/110`、真实 App XCUITest `3/3`、Watch `78/78`。
- Watch 已完成推荐成绩一键接受；手动确认按总杆 → 推杆 → Par 4/5 上球道/偏左/偏右 → 罚杆推进，Par 3 跳过球道结果。未记分时点下一洞先确认，历史洞编辑不改变当前实战洞。
- Watch 首页已有“记一杆”：先使用腕上 GPS 固定位置，再选择实际球杆或明确跳过；跳过仍保存位置，推荐杆不会自动写为实际杆。现有 Watch/phone adapters 把该位置映射到统一 live-round `location` 事件；已记非推杆数驱动“+2 推”的推荐成绩。
- Codex 已检查最终 run 的真实 watchOS 模拟器截图：首页 `第7洞 · P4` 不再截断；推荐确认、手动和取消均在首屏；Fairway 只有三种合法结果；Club Prompt 明确显示“建议”和“跳过球杆 · 保存位置”。seed/restore 使用不同 PID `55368 → 55928` 且状态一致。
- 里程碑 2 仍为 `CURRENT`：尚缺“上一洞未确认时已经捕获下一洞候选首杆”的真实 producer 与确认/取消后的归属闭环。当前仓库没有可复用的 provisional/hole-transition producer；不能用未接线的状态类型冒充完成。
