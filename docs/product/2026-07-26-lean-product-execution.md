...
| UI-03 | `ACTIVE / 语义已修，待真实图复验` | `e822827`/`c0929ad` 已为沙坑和水域统一接通真实障碍前后沿：`到 N` 是当前位置到前沿，`过 N` 是从当前位置越过后沿所需的总 carry；`sideM` 只留作内部风险几何，绝不面向球员显示。旧缓存只降级展示其能证明的读数，不把历史横向距离伪装成后沿；待新后端上的真实 Cypress Tee/球道中段截图复验后关闭。 |
| UI-04 | `QUEUED` | 真实 App 的 Watch 根页和地图仪表仍保留 watchOS 时钟与顶部安全区，而批准图是全屏仪表；结果是同一地图在运行态被整体压矮，底部操作也更容易相互挤占。UI-03 关闭后，只对赛中沉浸仪表验证隐藏持久系统覆盖，不让开局、列表或确认流程丢失系统导航语义。 |
