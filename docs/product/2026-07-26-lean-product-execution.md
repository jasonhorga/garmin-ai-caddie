...
| E2E-03 | `FIX READY / 待隔离凭证运行验证` | 只读生产审计确认 owner 历史当前 `461` 轮，其中 2026-07-30 的 `5` 轮 Cypress Point 均为 `55/75`、每轮对应唯一 `watch-*` round 且恰有 `56` 条事件；根因是 Watch runtime 用 owner admin token 完成真实 `/finish`。当前 RED→GREEN 配置测试要求 workflow 只读 `AI_CADDIE_CI_PLAYER_TOKEN`，Watch 测试入口以 Bearer 写入独立 player 分区且不注入 admin token。尚未删除或修改这 5 轮；创建 CI player、写入 GitHub secret 后才允许再次跑真实旅程。 |
