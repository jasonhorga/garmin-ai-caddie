...
| Web HMB owner run | Homeserver owner-only Playwright (`REVIEW_ROUND_REF=17603881`; no GitHub run/artifact was created) | Overview/detail/shotmap/topo HTTP 200; shotmap `found=true`, `globalId=6022`; six non-empty 1440x980 PNGs, 116,548--177,241 bytes; admin secret scan passed | Approve each image's map/topo, HMB round selection and first-frame state |
...
The homeserver owner-only run produced these six files. They are listed for
