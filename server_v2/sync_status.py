    ...
    *, root: Path = ROOT, data_mode: DataMode | None = None
    ...
    if persisted:
        last_run = SyncLastRunStatus(
            ...
            remoteRoundCount=_optional_int(persisted.get("remoteRoundCount")),
            remoteLatestRoundId=(
                str(persisted.get("remoteLatestRoundId"))
                if persisted.get("remoteLatestRoundId") is not None
                else None
            ),
            remoteLatestRoundAt=_iso_z(persisted.get("remoteLatestRoundAt")),
            newRoundCount=_optional_int(persisted.get("newRoundCount")),
        )
